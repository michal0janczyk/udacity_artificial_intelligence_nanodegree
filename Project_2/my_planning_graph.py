...
from layers import BaseActionLayer, BaseLiteralLayer, make_node, makeNoOp
import itertools
...
class ActionLayer(BaseActionLayer):
    def _inconsistent_effects(self, actionA, actionB):
        ...
        layers.ActionNode
        """
        # TODO: implement this function _inconsistent_effects
    ...
    def _interference(self, actionA, actionB):
        """ Return True if the effects of either action negate
        the preconditions of the other
        ...
        layers.ActionNode
        """
        # TODO: implement this function _interference
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        # TODO: implement this function _competing_needs
        for preconditionA, preconditionB in itertools.product(
                actionA.preconditions, actionB.preconditions):
            if self.parent_layer.is_mutex(
                    preconditionA, preconditionB) or self.parent_layer.is_mutex(
                    preconditionB, preconditionA):
                return True
        return False
...
class LiteralLayer(BaseLiteralLayer):
    def _inconsistent_support(self, literalA, literalB):
        ...
        layers.BaseLayer.parent_layer
        """
        # TODO: implement _inconsistent_support
        literalA_actions = []
        literalB_actions = []

        for parent_action in self.parent_layer.children:
            if literalA in parent_action.effects:
                literalA_actions.append(parent_action)

        for parent_action in self.parent_layer.children:
            if literalB in parent_action.effects:
                literalB_actions.append(parent_action)

        for actionA, actionB in itertools.product(
                literalA_actions, literalB_actions):
            if not self.parent_layer.is_mutex(
                    actionA, actionB) and not self.parent_layer.is_mutex(
                    actionB, actionA):
                return False
        return True
...
class PlanningGraph:
    def __init__(self, problem, state, serialize=True, ignore_mutexes=False):
        ...
        self._actionNodes = no_ops + [make_node(a)
    ...
    def h_levelsum(self):
        ...
        """
        # TODO: implement this function h_levelsum
        level_goal = set()
        level_sum = 0

        while not self._is_leveled and not level_goal == self.goal:
            for single_goal in self.goal - level_goal:
                if single_goal in self.literal_layers[-1]:
                    level_sum += len(self.literal_layers) - 1
                    level_goal.add(single_goal)
            self._extend()
    ...
    def h_maxlevel(self):
        ...
        """
        # TODO: implement maxlevel heuristic h_maxlevel
        level_goal = set()
        level_max = 0
        ...
        while not self._is_leveled and not level_goal == self.goal:
            for single_goal in self.goal - level_goal:
                if single_goal in self.literal_layers[-1]:
                    level_max = len(self.literal_layers) - 1
                    level_goal.add(single_goal)
            self._extend()
    ...
    def h_setlevel(self):
        ...
        Notes
        -----
        WARNING: you should expect long runtimes using this heuristic
        on complex problems
        """
        # TODO: implement setlevel heuristic h_setlevel
        import itertools
